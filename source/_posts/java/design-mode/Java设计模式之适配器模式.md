...
![](https://cdn.jsdelivr.net/gh/removeif/blog_image/20190620161658.png)
...
/*
*上面给出的是目标角色的源代码，这个角色是以一个JAVA接口的形式实现的。可以看出，这个接口声明了两个方法：
*sampleOperation1()和sampleOperation2()。而源角色Adaptee是一个具体类，它有一个sampleOperation1()方法，但是没有sampleOperation2()方法。
*/
...
![](https://cdn.jsdelivr.net/gh/removeif/blog_image/20190620161716.png)
...
1. **类适配器**，使用对象继承的方式，是静态的定义方式；**对象适配器**，使用对象组合的方式，是动态组合的方式。
...
过多的使用适配器，会让系统非常零乱，不易整体进行把握。比如，明明看到调用的是**A接口**，其实内部被适配成了**B接口**的实现，一个系统如果太多出现这种情况，无异于一场灾难。因此如果不是很有必要，可以不使用适配器，而是直接对系统进行重构。
...
/**
*鲁智深的故事
*　　和尚要做什么呢？吃斋、念经、打坐、撞钟、习武等。如果设计一个和尚*接口，
*给出所有的和尚都需要实现的方法，那么这个接口应当如下：
*/
...
*以史为鉴，可以知天下。研究一下几百年前鲁智深是怎么剃度成和尚的，会对Java编程有很大的启发。
*不错，当初鲁达剃度，众僧说：“此人形容丑恶、相貌凶顽，不可剃度他",但是长老却说：
...
![](https://cdn.jsdelivr.net/gh/removeif/blog_image/20190620161736.png)
